...
def bulk_insert_other_games(db: Session, games: list[dict]):
    for g in games:
        ...
        if existing:
            existing.numbers = g["numbers"]
            existing.extra = g.get("extra")
            existing.drawing_date = g.get("drawing_date")
        else:
...
def get_other_games(db: Session, limit: int = 10):
    return (
        db.query(OtherGameResult)
        .order_by(desc(OtherGameResult.fecha))
        .limit(limit)
        .all()
    )
