...
def _ml_preds_for_score(juego: str, sorteo: Optional[str]) -> list:
    """Predicciones ML en una sesión propia (hilos separados = conexión SQLite propia)."""
    try:
        from backend.lottery_analyzer import generar_predicciones
        from backend.database import engine
        from sqlalchemy.orm import Session

        with Session(engine) as ml_db:
            resp = generar_predicciones(ml_db, juego, sorteo)
            if not resp:
                return []
            return list(resp.get("digitos", [])) + list(resp.get("pares", []))
    except Exception:
        return []


def _score_numbers(db: Session, juego: str, sorteo: Optional[str], numeros: list[int],
                   limite: int = 15,
                   set_calientes: set = None, set_posibles: set = None) -> list[dict]:
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as ex:
            fut = ex.submit(_ml_preds_for_score, juego, sorteo)
            try:
                ml_preds = fut.result(timeout=5)
                ml_map = {p["numero"]: p["probabilidad"] for p in ml_preds}
    ...
    if set_calientes is None:
        set_calientes = set()
    if set_posibles is None:
        set_posibles = set()
    ...
    for n in numeros:
        ...
        if n in set_calientes and n in set_posibles:
            cat = "ambos"
        elif n in set_calientes:
            cat = "caliente"
        elif n in set_posibles:
            cat = "posible"
        else:
            cat = "discriminante"
...
def comparar_y_reducir(
    ...
    if db and juego:
        if modo == "completo" and alrededor:
            result["scored_completo"] = _score_numbers(
                db, juego, sorteo, alrededor, limite,
                set_calientes=set_calientes, set_posibles=set_posibles,
            )
        elif modo == "reducir" and discriminante:
            result["scored_final"] = _score_numbers(
                db, juego, sorteo, discriminante, limite,
                set_calientes=set_calientes, set_posibles=set_posibles,
            )
