...
if not tier_info or tier_info.get("tier") not in ("pro", "lifetime"):
    ...
    <div style="max-width:500px;margin:3rem auto;text-align:center;padding:3rem;background:linear-gradient(135deg, #1e3a5f, #2d1b4e);border-radius:1rem;border:1px solid #4a3f6b;">
        ...
        <p style="color:#94a3b8;margin:1rem 0;">Las Estadísticas Detalladas son una funcionalidad exclusiva para usuarios <strong style="color:#fbbf24;">Pro</strong> y <strong style="color:#8b5cf6;">De por Vida</strong>.</p>
...
with col_filtros1:
    juego = st.selectbox("Juego", ["Todos", "Pick 3", "Pick 4"], key="est_juego")
...
with col1:
    ...
    freqs = api_get("/api/estadisticas/frecuencias", {"juego": juego_param, "sorteo": sorteo_param, "dias": dias})
    if not freqs:
...
with col2:
    ...
    atrasados = api_get("/api/estadisticas/atrasados", {"juego": juego_param, "sorteo": sorteo_param})
    if not atrasados:
        st.info("⏳ Cargando datos...")
    else:
